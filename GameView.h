...
int letterToPlayer(char c);		// Might put this in a helper.c later. 
int min(int a, int b); 			// Return the smaller integer 
PlaceId strToId (char *string, int index);	// Same thing as obtainPlaceId, but takes a str
node getNthNode(node lastnode, int n);		// Gets the node n nodes away from the latest node 

////////////////////////////////////////////////////////////////////////
// Additional Interface Helper Functions
PlaceId obtainPlaceId(GameView gv, int i);
